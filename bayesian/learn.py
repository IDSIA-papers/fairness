...
def learn_bayesian_network(
    ...
    # or f"{df_name}_{learning_method}.pkl" not in os.listdir(
    # f"{save_path}/"
    # ):
    ...
    if "must_have_arcs" in learning_params:
        for arc in learning_params["must_have_arcs"]:
            src, dest = arc  # from target to sensible feature
    ...
    if show:
        ...
        display_and_save_gum_bn(
            bn, df_name, learning_method, save_path or ".", prefix=""
        )
...
def display_and_save_gum_bn(
    ...
    if True:
        ...
        gum.saveBN(bn, f"{save_path}/{df_name}_{prefix}_{learning_method}.pkl")

        gumimage.export(
            bn,
            f"{save_path}/{df_name}_{prefix}_{learning_method}.pdf",
            nodeColor=colors,
        )

        logger.success(
            f"Bayesian network image saved to {save_path}/{df_name}_{prefix}_{learning_method}.pdf"
        )
